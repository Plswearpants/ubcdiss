...
There are 2 primary shortcomings with this approach: one is due to a small spatial range of the cropping, the resolution in q-space is very much limited, making it hard to resolve features with shorter wavelengths. Second is that the cropped area could still preserve features from other defect sources, this is particularly problematic when we have quasiparticles with longer lifetimes; which induce weakly damped standing waves whose decaying tail could easily spread tens of unit cells, making it hard to avoid when cropping. 

\begin{figure}
	\includegraphics[width= \textwidth]{ZrSiS.png} 
	\centering
	\caption[\textbf{Selective band scattering in ZrSiS}]{\textbf{Selective band scattering in ZrSiS}. On the left, the \ac{FT} of the entire grid map (shown in the inset) reveals band scattering with various features. 
		Panels a)--d) show the selective band scattering of different defects, obtained by cropping around individual defect QPI patterns and taking the \ac{FT}. 
		Due to the limited spatial extent of the cropped images, the resulting $q$-space maps have reduced resolution. 
		Reproduced with permission from Butler \textit{et al.}, \textit{Phys. Rev. B} \textbf{96}, 195125 (2017)~\cite{butlerQuasiparticleInterferenceZrSiS2017}. 
		\textcopyright{} 2017 American Physical Society.}
	\label{fig:ch6_ZrSiS}
\end{figure}
