...
To adopt a direct method as we discussed, we will update each input to minimize the objective function while assuming the other input fixed. We now solve: 
\begin{equation}
	\label{sbdalgorithm}
...
In summary, we crafted a new algorithm that aims to reliably decompose contributions from different types of \ac{QPI} patterns. In the next two chapters, we are going to evaluate the algorithm's performance by testing it on both synthetic and experimental data. 

\begin{algorithm}
	\label{algo:MTSBDalgo}
