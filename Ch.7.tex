...
Impurity scattering by CO adsorbates on Ag(111) spans a wide energy range. We can thus apply the algorithm to the corresponding energy range of the grid map, and show examples of the resulting reconstructions near the Fermi level in Figure. \ref{fig:Ag2}. It is worth noting that while running the algorithm on multiple slices, the activation map across different slices are identical for a single type of defect; since we get a perfect reconstruction at the slice at Fermi level(0 meV), result from other slices are guaranteed to be high-quality and thus trust-worthy. We can thus construct a 3D kernel across both spatial and energy dimensions. This allows us to access energy dispersion of QPI features free from speckles, which can be clearly illustrated by comparing the second and third row of Figure. \ref{fig:Ag2}.
...
\subsection{ZrSiTe}
msg: 
1. success run of the single slice:
	a. MC-SBD run on two kernels. overview
	b. activation perfect match 
	c. defect dependent scattering features.
2. success run on whole block of data:
	a. defect scattering energy dispersion.
