...
\section{Data taking guideline for MC-SBD}
We showed that the algorithm adapts well on various datasets. However, it is worthwhile to establish a guideline on how to take QPI-STM data that maximizes the success rate of the algorithm. 

The dataset phase spaces shown in Figure. \ref{fig:phase_space} and \ref{fig:phase_spaceN=3} give quantitative references on the data taking process; if no additional complexities exists, one can expect the algorithm to produce perfect reconstruction in the regions defined by green markers. A safer region would be the box defined within XXX. Then it is to eliminate anomalies in the experimental datasets. Apart from the standard practice to minimize vibration and electric noise level in STM, we should be extra careful and minimize structured noise. 
...
rules:
1. Higher SNR 
2. More defect occurrence -> if low in density, need to include more areas. 
3. stable tip, especially need to avoid structured noise 

the guideline should be hierarchical, first, increase the SNR, then find a grid area on which there are more than 10 occurrence of defect for each type. 
