...
A quasiparticle is any long-lived, well-defined excitation in a system that behaves like a particle; it is a semi-classical term used to describe particle interactions in terms of renormalized single-particle excitation. In Fermionic systems, it corresponds to a sharp peak in the spectral function~\cite{bruusManyBodyQuantum2004}: 
	\[
...
For an adiabatic procedure to hold, time scale $\zeta$ in Equation \ref{adiabatic} should be bounded by two conditions: 1. rate of evolution should be small compared to the energy of the state: $\zeta \ll \epsilon_{k,\sigma}$ (we set $\hslash$ as unity), 2. the final bound state remains the same as the initial one, meaning the lifetime $\tau_{QP}$ of the bound state should be long enough: $\tau_{QP} \gg \zeta^{-1}$. Therefore, $\zeta$ should suits: 
\begin{equation}
	\tau_{QP}^{-1} \ll \zeta \ll \epsilon_{k,\sigma},
\end{equation}
...
We then expand the self-energy into real and imaginary parts, the effective single-electron energy is $\epsilon'_{k} = \epsilon_k + \operatorname{Re}\Sigma^R(k,\omega)$. We can then define the renormalized Fermi momentum $k^*_{F}$ as $\epsilon'_{\tilde{k_{F}}} = 0$. It is worth noting that, according to the Luttinger's theorem, in an isotropic Fermi liquid at a fixed density, Fermi momentum is protected and remain unrenormalized. However, in systems with interactions that induce a spontaneous symmetry breaking, the Fermi surface can be deformed and is no longer isotropic. This introduces an angular dependence to the Fermi wave vector and thus effectively renormalizes it. This instability of the Fermi surface is known as the Pomeranchuk instability, and is experimentally observed in quantum Hall systems, Sr\textsubscript{3}Ru\textsubscript{2}O\textsubscript{7}, and iron-based superconductors~\cite{fradkin_nematic_2010,chubukov_fermi-liquid_2018}.

In the absence of magnetic field, we then expand $G^R_{k, \omega}$ in equation \ref{retardedgreen} around $k = k^*_{F}$ and get: 
...
\noindent here, $\epsilon^*_{\mathbf{k}}$ is the renoramlized energy, $\Gamma_{\mathbf{k}}$ is the relaxation rate which is inverse proportional to the quasiparticle lifetime $\tau_{QP}$, and $Z$ is the quasiparticle residue, or weight factor of the qausiparticle. We can further parameterize the renormalized energy with the effective mass $m^*$: 
...
where  $\epsilon(k)$ is the renormalized energy dispersion of the Bloch state for different $\vec{k}$; Remark: from now on all the renormalized energy will be written directly as $\epsilon(k)$ unless otherwise mentioned. And if we plug Equation \ref{eq:bloch} into Equation \ref{eq:ldos}, we see that the $\text{LDOS}$ is translationally invariant at all Bravais-lattice-equivalent points. Thus it is why in ideal case, real space imaging technique like \ac{STM}, can not be used to measure $\epsilon(k)$. 

However, crystal imperfections can help us introduce a momentum texture to the $\text{LDOS}(E, \vec{r})$. In low temperature STM, where the phonon modes and other excitations are rare or absent, lattice imperfections such as point defects can cause elastic scattering which mixes electronic eigenstates at different $\vec{k}$ but at the same energy. The interference between the incoming and outgoing wave forms a "standing wave" that acts as a real space modulation in $\text{LDOS}(E)$. At the Fermi energy, this pattern is described as a Friedel oscillation~\cite{benaFriedelOscillationsDecoding2016}. This spatial modulation in $\text{LDOS}$ is the QPI pattern that we refer to in \ac{STM} studies. We can perform \ac{STS} measurement on the region that presents this QPI pattern, then apply a Fourier transform to the STS, allowing us to view this QPI pattern in the momentum space; the q-space QPI pattern is directly related to the quasiparticle band strucutre. This whole process of detecting QPI pattern and analyze it in momentum space is what we call a QPI measurement. 
...
and therefore: 
\begin{align}
	\delta\rho\left(\mathbf{x},\omega\right) &= - \frac{1}{\pi} \operatorname{Im}\left(\frac{G_0^2(\mathbf{x},\mathbf{x_d};\omega)}{E_d^{-1} - G_0(\mathbf{x_d},\mathbf{x_d};\omega)}\right) \label{singleldos}. 
\end{align}
...
The setup of simulation is illustrated in Fig. \ref{fig:ch5_qpisim_demo}, a grid of $128\times128$ in real space covering a $20\times20$ lattice space indicated in orange dots. We can implant defects on different lattice sites, indicated in green dots. 

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{ch5_singe_simulation.pdf} % Replace with your image file
	\caption[\textbf{single defect QPI pattern simulation}]{\textbf{single defect QPI pattern simulation}: $\delta\rho(\textbf{x},\omega)$ for single defect at the center of the 51 by 51 lattice grid with V$_\textbf{d}$ = -0.02, t = -0.2 and $\omega$ = 0.45, 0.20, -0.05 and -0.45\,eV for a)-d), respectively; the integrand size is defined by n = 300, $\epsilon = 1*10^{-3}$. Contrast for each image is self-normalized to [0,1].}
...
As mentioned in Chapter \ref{chapter:STM}, taking a grid map is very involved. A successful grid map requires both an ideal instrumental setup and a set of proper measurement parameters based on the understanding of the target material. 
...
\subsection{multi-defect QPI pattern and the phase noise} \label{sec: multi-defect QPI theory}
