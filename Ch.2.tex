\chapter{Scanning Tunneling Microscopy (STM)} \label{chapter:STM}
...
"The way to derive Bardeen's tunneling theory is not straightforward; it does not proceed by routine application of textbook techniques"~\cite{gottliebBardeensTunnellingTheory2006a}. Here I follow the approach of Gottlieb and Wesoloski~\cite{gottliebBardeensTunnellingTheory2006a}, which was mostly inspired by C.~B.~Duke \cite{dukeTunnelingSolids1973}. The readers are encouraged to read the full derivation in this comprehensive and accessible tutorial by Gottlieb and Wesoloski~\cite{gottliebBardeensTunnellingTheory2006a}. 
...
\begin{equation}
	I = \frac{4\pi e}{\hslash} \int_0^{eV}|M_{\text{tip}-\text{sample}}|^2 \rho_t(\epsilon - eV) \cdot \rho_s(\epsilon) d\epsilon.
\end{equation}
