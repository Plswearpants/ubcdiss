...
During the development of the \ac{MC-SBD} algorithm, I used generative AI tools, specifically ChatGPT and Cursor to assist in several ways:
\begin{itemize}
	\item Literature Study(with ChatGPT): I used ChatGPT to help me understand key concepts from the relevant literature and clarify technical points while studying prior work.
	\item Algorithm Development(with Cursor): After the architecture of the algorithm was co-designed by Markus Altthaler and me, I prototyped the core functions. During the iteration phase, I used Cursor as an interactive companion to write test cases, explore alternative implementations, and refactor code for improved readability and maintainability.
	\item Performance Optimization(with Cursor): In the implementation phase, I used Cursor to assist in building parallelized loops and optimizing code execution, which significantly improved runtime—particularly for running the synthetic dataset sweeps that produced Figure~\ref{fig:phase_space}.
\end{itemize}
